...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "namud-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  namud-remote version")
	_, _ = fmt.Fprintln(w, "  namud-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  namud-remote cli <command> [args...]")
}
...
// runCLI is a thin CLI that sends a single RPC to a running namud-remote
// server via its stdio pipe. Useful for testing and scripting.
func runCLI(args []string) int {
	if len(args) == 0 {
		_, _ = fmt.Fprintln(os.Stderr, "cli requires a command")
		return 2
	}
	method := args[0]
