...
/// Renders a single pane leaf — terminal or browser — with a focus highlight border.
private struct PaneLeafView: View {
    ...
    var body: some View {
        ZStack {
            ...
            // Focus border on the active pane
            if isFocused {
    ...
    private var panelContent: some View {
        ...
        case .terminal:
            if let panel = panelManager.panel(for: leaf.id) {
                TerminalView(panel: panel, onFocus: {
                    panelManager.focusPanel(id: leaf.id)
                }, isActive: isActive)
            } else {
                Color.black
            }
