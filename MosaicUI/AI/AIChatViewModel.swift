...
final class AIChatViewModel: ObservableObject {
    ...
    func send(_ text: String) async {
        ...
        let reply = await mosaicAI.send(text, conversationID: conversationID)
    ...
    private func loadActiveProvider() {
        // If MosaicAI already has a provider (e.g. from env var), use it
        if mosaicAI.hasProvider {
            return
        }
