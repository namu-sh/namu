...
enum AIProvider: String, CaseIterable, Identifiable, Codable {
    ...
    var keychainService: String { "mosaic.ai.\(rawValue)" }
...
final class GatewayPreferencesStore: ObservableObject {
    ...
    private static let gatewayURLKey            = "mosaic.gateway.url"
    private static let pairingTokenService      = "mosaic.gateway"
...
final class AIPreferencesStore: ObservableObject {
    ...
    private static let providerKey     = "mosaic.ai.provider"
    private static let modelKey        = "mosaic.ai.model"
    private static let safetyKey       = "mosaic.ai.safetyLevel"
    private static let streamKey       = "mosaic.ai.streamResponses"
    private static let maxTokensKey    = "mosaic.ai.maxTokens"
...
struct AIPreferencesView: View {
    ...
    private var gatewayTab: some View {
        Form {
            ...
            Section("Link Telegram") {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        Label("1. Start the Mosaic Gateway with your Telegram bot token", systemImage: "terminal")
            ...
            // Unlink Instructions
            Section("Unlink") {
                Text("To unlink your Telegram account, send /unlink to the bot. This removes the connection between your Telegram chat and Mosaic.")
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
