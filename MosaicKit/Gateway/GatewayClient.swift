...
/// WebSocket client that connects to the Mosaic Gateway server.
///
/// Responsibilities:
...
final class GatewayClient: ObservableObject {
    ...
    /// Validates X-Mosaic-Signature header against the message body.
    /// The gateway server must include `X-Mosaic-Signature: <hex-HMAC-SHA256>` as a
