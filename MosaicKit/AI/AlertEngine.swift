...
final class AlertEngine: @unchecked Sendable {
    ...
    func start() {
        let id = eventBus.subscribe(
            events: [.processExit, .outputMatch, .portChange, .shellIdle]
        ) { [weak self] data in
    ...
    private func handleRawEvent(_ data: Data) {
        guard let notification = try? JSONDecoder().decode(JSONRPCNotification.self, from: data),
              let event = MosaicEvent(rawValue: notification.method) else { return }
    ...
    private func matches(rule: AlertRule, event: MosaicEvent, params: [String: JSONRPCValue]) -> Bool {
        switch rule.trigger {
...
extension AlertEngine {
    private static let userDefaultsKey = "mosaic.alertRules"
