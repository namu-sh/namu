...
final class SocketServer: @unchecked Sendable {
    ...
    struct Config: Sendable {
        ...
        static func defaultPath(tag: String? = nil) -> Config {
            if let tag {
                return Config(socketPath: "/tmp/mosaic-\(tag).sock")
            }
            return Config(socketPath: "/tmp/mosaic.sock")
        }
    }
    ...
    func start() throws {
        ...
        try stateLock.withLock {
            ...
            DispatchQueue(label: "com.mosaic.socket-accept", qos: .utility).async { [weak self] in
    ...
    private func runAcceptLoop(generation: UInt64) {
        while shouldContinue(generation: generation) {
            ...
            DispatchQueue(label: "com.mosaic.socket-client.\(clientFD)", qos: .utility).async { [weak self] in
    ...
    private func handleSubscribe(
        ...
        var eventSet: Set<MosaicEvent> = Set(MosaicEvent.allCases)
        if let obj = try? JSONDecoder().decode(JSONRPCRequest.self, from: data),
           ...
           case .array(let arr) = params["events"] {
            ...
            let parsed = names.compactMap { MosaicEvent(rawValue: $0) }
