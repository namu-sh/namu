...
final class AICommands {
    ...
    private let mosaicAI: MosaicAI
    private let conversationManager: ConversationManager

    // MARK: - Init

    init(workspaceManager: WorkspaceManager, eventBus: EventBus, mosaicAI: MosaicAI, conversationManager: ConversationManager) {
        self.workspaceManager = workspaceManager
        self.eventBus = eventBus
        self.mosaicAI = mosaicAI
    ...
    //   session_id   (string, optional) — conversation session UUID; passed to MosaicAI
    //
    // Returns:
    //   session_id   string
    ...
    private func message(_ req: JSONRPCRequest) async throws -> JSONRPCResponse {
        ...
        let reply = await mosaicAI.send(content, conversationID: conversationID)
    ...
    private func status(_ req: JSONRPCRequest) async throws -> JSONRPCResponse {
        ...
        return .success(id: req.id, result: .object([
            ...
            "processing":    .bool(mosaicAI.isProcessing)
        ]))
    }
